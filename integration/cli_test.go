...
func TestIntegration(t *testing.T) {
	...
	t.Run("windows service lifecycle", func(t *testing.T) {
		...
		cfgPath := writeLocalConfig(t, fakeClient)
		sushiBinary := buildSushiBinary(t, repoRoot)
		_, _ = runSushiServiceBinary(t, repoRoot, sushiBinary, "uninstall", cfgPath)

		out, err := runSushiServiceBinary(t, repoRoot, sushiBinary, "status", cfgPath)
		if err == nil {
		...
		installOut, installErr := runSushiServiceBinary(t, repoRoot, sushiBinary, "install", cfgPath)
		if installErr != nil {
			...
			for _, cmd := range []string{"start", "stop", "status", "uninstall"} {
				out, err := runSushiServiceBinary(t, repoRoot, sushiBinary, cmd, cfgPath)
				if err == nil {
		...
		t.Cleanup(func() {
			_, _ = runSushiServiceBinary(t, repoRoot, sushiBinary, "stop", cfgPath)
			_, _ = runSushiServiceBinary(t, repoRoot, sushiBinary, "uninstall", cfgPath)
		})
		...
		out, err = runSushiServiceBinary(t, repoRoot, sushiBinary, "start", cfgPath)
		...
		out, err = runSushiServiceBinary(t, repoRoot, sushiBinary, "stop", cfgPath)
		...
		out, err = runSushiServiceBinary(t, repoRoot, sushiBinary, "uninstall", cfgPath)
		if err != nil {
...
func buildSushiBinary(t *testing.T, root string) string {
	t.Helper()
	out := filepath.Join(t.TempDir(), "sushi")
	if runtime.GOOS == "windows" {
		out += ".exe"
	}
	cmd := exec.Command("go", "build", "-o", out, "./cmd/sushi")
	cmd.Dir = root
